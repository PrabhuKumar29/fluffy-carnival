...
def user_register():
    if request.method=="GET":
        ...
        user_mail = request.form.get('mailid')
        username = request.form.get('name')
        password = request.form.get('pass')
        conn = db.get_db()
        cur = conn.cursor()
        bcrypt = Bcrypt(current_app)
        hashed_pass = bcrypt.generate_password_hash(p).decode('utf-8')
        pas = hashed_pass
        cur.execute("Insert into users(name, mail, pass) values (%s, %s, %s)", (user_mail, username, password))
        cur.close()
        conn.commit()
...
def user_login():
    if request.method=="GET":
        ...
        username = request.form.get("mailid")
        ...
        try:
          ...
          if bcrypt.check_password_hash(pas, pwd):
            cur = conn.cursor()
            cur.execute("select id from users where mail=%s", (username,))
            uid = cur.fetchone()[0]
            cur.close()
